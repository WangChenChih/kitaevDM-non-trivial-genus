...
\begin{figure}
    ...
    \caption{(a) The web-patch configuration $\mathfrak{H}$ covering the interior hole of a non-simply connected subsystem. Its corresponding string operator $\Sigma_\mathfrak{H}$ commutes with other string operators lying within the shaded subsystem. (b) The non-contractible Wilson operator with two endpoints attached to different boundaries anti-commutes with the web-patch operator $\Sigma_\mathfrak{H}$ covering the interior hole, while the non-contractible Wilson operator with both endpoints attached to the web-patch commutes with $\Sigma_\mathfrak{H}$.}
...
\begin{equation}\label{eq:patition-condition}
  \sum_{\lrg{s}} \tilde{\mathfrak{C}}_\mathfrak{D}^{(\lrg{s})} = \mathfrak{C}_\mathfrak{D}
\end{equation}
